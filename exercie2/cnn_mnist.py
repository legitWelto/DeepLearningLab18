...
def train_and_validate(x_train, y_train, x_valid, y_valid, num_epochs, lr, num_filters, filter_size, batch_size):
    ...
    hist = model.fit(x=x_train, y=y_train, batch_size=batch_size, epochs=num_epochs, validation_data=(x_valid, y_valid))
    learning_curve = hist.history['val_error']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", default=1e-2, type=float, nargs="?", help="Learning rate for SGD")
    ...
    parser.add_argument("--filter_size", default=3, type=int, nargs="?",
                        help="The size of the filters for each convolution layer")
